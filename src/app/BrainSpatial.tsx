  ...
  useEffect(() => {
    if (selectedSample && gene) {
      const fetchConfig = async () => {
        const configPath: string = `https://users.wenglab.org/kresgeb/psych_encode/${selectedSample.dataset}/configs/${selectedSample.internalFileName}/config.json`;
  ...
  return (
    <Stack>
      <Autocomplete
        ...
        sx={{ width: 300 }}
      />
      {config && <Vitessce height={800} config={config} theme="light" />}
    </Stack>
  );
};
